...
#SBATCH -A YOUR_PROJECT   # replace with your OLCF allocation
#SBATCH -J warpx
#SBATCH -t 00:10:00
#SBATCH -p batch
...
# ---- HDF5 parallel I/O on Lustre (Orion) ----------------------------------
# BTD file-based encoding creates one .h5 per snapshot. Snapshot 0 was created
# OK but snapshot 1 failed at Cray MPI-IO's per-file Lustre stripe call
# (ADIO_CRAY_ADIO_OPEN: llapi_layout_stripe_size_set failed: Invalid argument),
# which openPMD reports as "Failed to create HDF5 file" and then aborts.
# Force a known-valid stripe for EVERY .h5 file instead of a bad computed one.
# 16 MiB = 16777216 is a valid multiple of 64 KiB; 8 OSTs is safe on Orion.
export MPICH_MPIIO_HINTS="*.h5:striping_unit=16777216:striping_factor=8:romio_cb_write=enable:cb_nodes=8"
# Print the hints actually applied per file, so file-0 vs file-1 can be compared
# if this still fails (look for striping_unit/striping_factor in the .h5 lines).
STR=$(( SLURM_JOB_NUM_NODES < 32 ? SLURM_JOB_NUM_NODES : 32 ))
export MPICH_MPIIO_HINTS="*.h5:striping_unit=16777216:striping_factor=${STR}:romio_cb_write=enable:cb_nodes=${STR}"
...
export BPLS=${pwd}/../../../../warpx_tests/bpls
export EXE=${pwd}/../../../../warpx_tests/EXE
...
useConfig()
{
    ...
    INPUTS=${pwd}/../../../../warpx_tests/inputs_3d/opmd/btd_${key}/${fileType}/input.n${SLURM_NNODES}${ENC}
