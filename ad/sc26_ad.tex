...
\item[$C_3$] \texttt{FlattenSteps} for lab-frame BTD snapshots from non-contiguous
  boosted-frame data at negligible write-time and reader-side metadata-stitching overhead.
...
\midrule
$A_1$ & $C_1$--$C_5$ &
  Table~I; Table~II; Fig.~2 (Perlmutter 3D); Figs.~3--5 (BTD);
...
the log timer extractor, CSVs for Table~I and Figs.~2--5, and
\texttt{bpls~-l} timing scripts and output for Table~II.
...
\begin{itemize}
  ...
  \item BTD: \texttt{FlattenSteps} write-time and reader-side metadata-stitching overhead negligible
        (Table~II: \texttt{bpls~-l} $\approx$0.12~s for both configs at all scales);
...
\begin{itemize}
  ...
  \item \path{read_metadata/}: \texttt{browse\_flatten.sh} and
        \texttt{browse\_default.sh} run \texttt{bpls~-l} 3$\times$ per snapshot
        on a login node; \texttt{out\_flatten} and \texttt{out\_default} contain
        raw timing output (36 trials each); \texttt{analyze.py} reproduces Table~II.
\end{itemize}
...
\path{fig_btd_b_span_async_frontier.csv} & Fig.~3 \\
\path{fig_btd_c_agg_perlmutter.csv} & Fig.~4 \\
\path{fig_btd_d_subfiles_perlmutter.csv} & Fig.~5 \\
\path{repeat_runs_raw.csv}, \path{repeat_summary.csv} & mean$\pm$stdev \\
\path{read_metadata/} & Table~II \\
\bottomrule
\end{tabular}
\end{center}
}
...
\begin{enumerate}
  \item \textbf{Build.}
    Frontier (HIP): \texttt{export WORKDIR=\path{/lustre/orion/<proj>/scratch/<u>/<exp>}};
    \texttt{source scripts/frontier/bash.setup.env};
    \texttt{./scripts/frontier/build.setup}.
    Perlmutter (CUDA): \texttt{export WORKDIR=\path{/pscratch/sd/<u>/<exp>}};
    \texttt{./scripts/perlmutter/build.setup} (loads NERSC modules).
  \item \textbf{Point and submit.}
    In \texttt{warpx\_tests/}, symlink \texttt{EXE} $\rightarrow$ WarpX and
    \texttt{bpls} $\rightarrow$ ADIOS2 \texttt{bpls}; set \texttt{\#SBATCH~-A}.
    Submit job scripts under \texttt{PerformanceRun/} (scale-out) or
    \texttt{repeat/} (3$\times$ small-$N$ repeats).
  \item \textbf{Extract.}
    From the parent of each \texttt{job\_id/}:
    \texttt{python3 scripts/frontier/extract\_time\_3.6.py job\_id}
    $\rightarrow$ TotalTime, IOTime.
    Option~1 I/O time:
    $\mathrm{TotalTime}_{\mathrm{I/O}}-\mathrm{TotalTime}_{\mathrm{nullcore}}$
    when nullcore runs exist; \textbf{DataSize} is the on-disk output size.
  \item \textbf{Aggregate.}
    Write times and sizes into \path{results/*.csv}; replot in Google Sheets.
\end{enumerate}
...
\artout
CSV throughput vs.\ nodes plus small-$N$ mean$\pm$stdev; plots of Figs.~2--5
from those CSVs; \texttt{bpls~-l} timings for Table~II.
Compare to Table~I, Table~II, and Figs.~2--5 on relative trends;
